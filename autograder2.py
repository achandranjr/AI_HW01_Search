...
def loadModuleFile(moduleName, filePath):
    """Load a module from a file path using importlib (replaces imp.load_module)."""
    spec = importlib.util.spec_from_file_location(moduleName, filePath)
    if spec is None:
        raise ImportError(f"Cannot find module '{moduleName}' at '{filePath}'")
    module = importlib.util.module_from_spec(spec)
    sys.modules[moduleName] = module  # register so inter-module imports resolve correctly
...
def evaluate(generateSolutions, testRoot, moduleDict, exceptionMap=ERROR_HINT_MAP,
    ...
    for q in test_subdirs:
        ...
        # filter() and map() return iterators in Python 3, so wrap in list/sorted
